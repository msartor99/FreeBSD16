...
TITLE="FreeBSD 16 Post-Installation (Idempotent)"
...
This script deeply modifies your FreeBSD 16 system configuration. \
...
EOF
    ...
    if [ -n "$USER_NAME" ]; then
        export USER_NAME
        # Create user if missing, or modify if exists
        if ! id "$USER_NAME" >/dev/null 2>&1; then
            pw useradd "$USER_NAME" -m -G wheel,operator,video -s /bin/sh -c "System Administrator"
        else
            pw usermod "$USER_NAME" -G wheel,operator,video
        fi
...
hardware_config() {
    ...
    # 1. Critical elements (X11, Display Manager, DBUS, Input drivers for FB16)
    ...
    # Enable FUSE, ext2fs & evdev kernel modules
    ...
    # Extended Devfs rules for FreeBSD 16 / Wayland / DRM
...
nvidia_config() {
    ...
    # SDDM requires video group permission to access Nvidia devices
...
apps_config() {
    ...
    # Removed chromium (often broken on 16-CURRENT)
...
iphone_config() {
    ...
    # 3. FUSE Security and user-mounting setup for FreeBSD 16
    add_kld_module "fusefs"
...
#!/bin/sh
# Easy FreeBSD 16 utility to mount and sync iPhone media
